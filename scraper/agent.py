...
''' download agent image from liquidpedia'''
...
''' download map image from liquidpedia'''
...
if __name__ == "__main__":
    with open("../data/map.json", mode='r', encoding='utf-8') as j:
        maps = json.load(j)
    for map in maps:
        print(map["mapName"])
        print(map["imageUrl"])
        if(map["imageUrl"] != ""):
            loadImage("https://liquipedia.net"+map["imageUrl"], "../data/images/maps/"+map["mapName"]+".png")
            map["imageUrl"]="/data/images/map/"+map["mapName"]+".png"
            time.sleep(3)
    with open("../data/maps2.json", "w") as f:
